...
describe('Booking Validator', () => {
  ...
  describe('validateCreateBooking', () => {
    ...
    it('should reject missing startTime', () => {
      ...
      expect(() => validateCreateBooking(body, now)).toThrow('startTime is required');
    });
    ...
    it('should reject missing endTime', () => {
      ...
      expect(() => validateCreateBooking(body, now)).toThrow(AppError);
      expect(() => validateCreateBooking(body, now)).toThrow('endTime is required');
    });
    ...
    it('should reject invalid date format', () => {
      ...
      expect(() => validateCreateBooking(body, now)).toThrow(AppError);
      expect(() => validateCreateBooking(body, now)).toThrow('valid ISO-8601 date');
    });
    ...
    it('should reject when startTime equals endTime', () => {
      ...
      expect(() => validateCreateBooking(body, now)).toThrow(AppError);
      expect(() => validateCreateBooking(body, now)).toThrow('startTime must be before endTime');
    });
    ...
    it('should reject bookings in the past', () => {
      ...
      expect(() => validateCreateBooking(body, now)).toThrow(AppError);
      expect(() => validateCreateBooking(body, now)).toThrow('Cannot create bookings in the past');
    });

    it('should reject non-object body', () => {
      expect(() => validateCreateBooking(null, now)).toThrow('must be a JSON object');
      expect(() => validateCreateBooking('string', now)).toThrow('must be a JSON object');
    });
  });
});
...
describe('Booking Service - Overlap Detection', () => {
  ...
  it('should throw 404 when cancelling non-existent booking', () => {
    ...
    expect(() => bookingService.cancelBooking('non-existent')).toThrow('not found');
  });
});
